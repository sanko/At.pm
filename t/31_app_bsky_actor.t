...
subtest 'live' => sub {
    ...
    isa_ok $bsky->getPreferences(), ['At::Lexicon::app::bsky::actor::preferences'], q[$bsky->getPreferences()];
    isa_ok $bsky->getProfile('atperl.bsky.social'), ['At::Lexicon::app::bsky::actor::profileViewDetailed'],
        q[$bsky->getProfile('atperl.bsky.social')];
    {
        my $ret = $bsky->getProfiles( 'atperl.bsky.social', 'bsky.app' );
        is scalar @{ $ret->{profiles} }, 2, q[$bsky->getProfiles( 'atperl.bsky.social', 'bsky.app' )];
        isa_ok $ret->{profiles}[0], ['At::Lexicon::app::bsky::actor::profileViewDetailed'], '...returned list of profileViewDetailed'
    }
    {
        my $ret = $bsky->getSuggestions();
        ok scalar @{ $ret->{actors} }, q[$bsky->getSuggestions( )];    # should be 50 but the default might change
        isa_ok $ret->{actors}[0], ['At::Lexicon::app::bsky::actor::profileView'], '...returned list of profileView'
    }
    {
        my $ret = $bsky->searchActorsTypeahead('perl');
        ok scalar @{ $ret->{actors} }, q[$bsky->searchActorsTypeahead( )];    # should be 10 but the default might change
        isa_ok $ret->{actors}[0], ['At::Lexicon::app::bsky::actor::profileView'], '...returned list of profileView'
    }
    {
        my $ret = $bsky->searchActors('perl');
        ok scalar @{ $ret->{actors} }, q[$bsky->searchActors( )];             # should be 25 but the default might change
        isa_ok $ret->{actors}[0], ['At::Lexicon::app::bsky::actor::profileView'], '...returned list of profileView'
    }
    ok $bsky->putPreferences( { label => 'test', visibility => 'hide', '$type' => 'app.bsky.actor.defs#contentLabelPref' } ),
        q[$bsky->putPreferences( { ..., '$type' => 'app.bsky.actor.defs#contentLabelPref' } )];
};
#
done_testing;
