...
package At::Lexicon::Parser 1.0 {
    ...
    sub lexicon($raw) {
        ...
        for my ( $key, $value )( %{ $raw->{defs} } ) {
            if ( $value->{type} ne 'object' ) {
                0 && warn 'Skipping ' . $key . ' because it is of type ' . $value->{type};
                next;
            }
            $key =~ s[[#\.]+][::]gm;
            my $package = 'At::Lexicon::' . $lexicon{package} . '::' . $key;

            # warn $package;
            my $metaclass = Object::Pad::MOP::Class->begin_class( $package, () );
            for my ( $prop_key, $prop_value )( %{ $value->{properties} } ) {
                ...
                my $metafield = $metaclass->add_field(
                    '$' .
                        ...
                        defined $prop_value->{default} ?
                            ( default => $prop_value->{type} eq 'boolean' ? !!$prop_value->{default} : $prop_value->{default} ) :
                            ( grep { $prop_key eq $_ } @{ $value->{required} // [] } ) ? () :
                            ( default => undef ),
                        param    => $prop_key,
                        accessor => $prop_key
                        )
                ...
                if ( $prop_value->{type} eq 'boolean' ) {
                    $metaclass->add_BUILD(
                        sub ( $self, %args ) {
                            $args{$prop_key} = !!$args{$prop_key} unless is_bool( $args{$prop_key} );
                        }
                    );
                }
                    ...
                    $metaclass->add_BUILD(
                        sub ( $self, %args ) {
                            ...
                            Carp::confess( sprintf q['%s' with value of %d is below the minimum value of %d],
                                $prop_key, $args{$prop_key}, $prop_value->{minimum} )
                    ...
                    $metaclass->add_BUILD(
                        sub ( $self, %args ) {
                            ...
                            Carp::confess( sprintf q['%s' is too long (%d vs %d max graphemes)], $prop_key, $len, $prop_value->{maxGraphemes} )
                    ...
                    $metaclass->add_BUILD(
                        sub ( $self, %args ) {
                            ...
                            Carp::confess( sprintf q[%s' is too long (%d vs %d max characters)], $prop_key, $len, $prop_value->{maxLength} )
                                if $len > $prop_value->{maxLength};
                        }
                    ) if defined $prop_value->{maxLength};
                    $metaclass->add_BUILD(
                        sub ( $self, %args ) {
                            if ( defined $prop_value->{knownValues} && !grep { $_ eq $args{$prop_key} } @{ $prop_value->{knownValues} } ) {
                                state $list //= sub {
                                    my @list = map {"'$_'"} @_;
                                ...
                                croak sprintf q[Field '%s' should be %s], $prop_key, $list->( @{ $prop_value->{knownValues} } );
                            }
                            state $format //= {
                                did => sub ($str) {
                                    At::Protocol::DID->new( uri => $str ) if defined $str;
                                },
                                handle => sub ($str) {
                                    At::Protocol::Handle->new( id => $str ) if defined $str;
                                },
                                datetime => sub ($str) {
                                    At::Protocol::Timestamp->new( timestamp => $str ) if defined $str;
                                },
                                uri => sub ($str) {
                            ...
                            if ( defined $prop_value->{format} ) {
                                if ( defined $format->{ $prop_value->{format} } ) {
                                    $args{$prop_key} = $format->{ $prop_value->{format} }->( $args{$prop_key} );
                                }
                                else {
                                    croak 'Unknown string format: ' . $prop_value->{format};
                                }
                            }
                    ...
                    $metaclass->add_BUILD(
                        sub ( $self, %args ) {
                            return if !defined $args{$prop_key};
                            croak $prop_key. ' should be an array' unless ref $args{$prop_key} eq 'ARRAY';
                        }
                    );
                    $metaclass->add_BUILD(
                        sub ( $self, %args ) {
                            return if !defined $args{$prop_key};
                            croak $prop_key . ' should be an array of ' . $prop_value->{maxLength} . ' or fewer elements'
                                if scalar @{ $args{$prop_key} } > $prop_value->{maxLength};
                        }
                    ) if defined $prop_value->{maxLength};
                    $metaclass->add_BUILD(
                        sub ( $self, %args ) {
                            return if !defined $args{$prop_key};
                            croak $prop_key . ' should be an array of ' . $prop_value->{minLength} . ' or more elements'
                                if scalar @{ $args{$prop_key} } < $prop_value->{minLength};
                        }
                    ...
                    if ( $prop_value->{items}{type} eq 'ref' ) {
                        ...
                        $metaclass->add_BUILD(
                            sub ( $self, %args ) {
                                return if !defined $args{$prop_key};
                                $args{$prop_key} = [ map { $ref->new(%$_); } @{ $args{$prop_key} } ]
                        ...
                        $metaclass->add_BUILD(
                            sub ( $self, %args ) {
                                ...
                                $args{$prop_key} = [
                                    map {
                                        require CBOR::Free;
                                        CBOR::Free::decode($_) unless blessed $_;
                                    } @{ $args{$prop_key} }
                                ];
                            }
                        );
                        ...
                        $metaclass->add_BUILD(
                            sub ( $self, %args ) {
                                return if !defined $args{$prop_key};
                                $args{$prop_key} = [ map { blessed $_ ? $_ : $ref->new(%$_); } @{ $args{$prop_key} } ];
                            }
                        );
                    }
                    elsif ( $prop_value->{items}{type} eq 'string' ) {
                        if ( !defined $prop_value->{items}{format} ) {    # Don't do anything.
                            $metaclass->add_BUILD(
                                sub ( $self, %args ) {
                                    return if !defined $args{$prop_key};
                                    Carp::confess( sprintf q[element in array '%s' is too long (%d max graphemes)],
                                        $prop_key, $prop_value->{items}{maxGraphemes} )
                                        if grep { At::_glength($_) > $prop_value->{items}{maxGraphemes} } @{ $args{$prop_key} };
                                }
                            ) if defined $prop_value->{items}{maxGraphemes};
                            $metaclass->add_BUILD(
                                sub ( $self, %args ) {
                                    return if !defined $args{$prop_key};
                                    Carp::confess( sprintf q[element in array '%s' is too long (%d max length)],
                                        $prop_key, $prop_value->{items}{maxLength} )
                                        if grep { length($_) > $prop_value->{items}{maxLength} } @{ $args{$prop_key} };
                                }
                            ...
                            $metaclass->add_BUILD(
                                sub ( $self, %args ) {
                                    require CBOR::Free;
                                    return if !defined $args{$prop_key};
                                    $args{$prop_key} = [ map { blessed $_ ? $_ : CBOR::Free::decode(%$_); } @{ $args{$prop_key} } ];
                                }
                            );
                        }
                            ...
                            $metaclass->add_BUILD(
                                sub ( $self, %args ) {
                                    require URI;
                                    return if !defined $args{$prop_key};
                                    $args{$prop_key} = [ map { blessed $_ ? $_ : URI->new($_); } @{ $args{$prop_key} } ];
                                }
                            );
                        }
                        else {
                    ...
                    $metaclass->add_BUILD(
                        sub ( $self, %args ) {
                            return if !defined $args{$prop_key};
                            return if blessed $args{$prop_key};
                            $args{$prop_key} = ( 'At::Lexicon::' . $ref )->new( %{ $args{$prop_key} } );
                        }
                    );
                }
                elsif ( $prop_value->{type} eq 'union' ) {
                    warn 'TODO: ' . $prop_value->{type};
                    ...
                    $metaclass->add_BUILD(
                        sub ( $self, %args ) {
                            return if !defined $args{$prop_key};
                            my $len = length( $args{$prop_key} );
                            Carp::confess( sprintf q[%s' is too long (%d vs %d max bytes)], $prop_key, $len, $prop_value->{maxLength} )
                                if $len > $prop_value->{maxLength};
                        }
                    ) if defined $prop_value->{maxLength};
                    ...
                    $metaclass->add_BUILD(
                        sub ( $self, %args ) {
                            return if !defined $args{$prop_key};
                            require CBOR::Free;
                            $args{$prop_key} = CBOR::Free::decode( $args{$prop_key} );
                        }
                    );
                }
                    ...
                    $metaclass->add_BUILD(
                        sub ( $self, %args ) {
                            return if !defined $args{$prop_key};
                            my $len = length( $args{$prop_key} );
                            Carp::confess( sprintf q[%s' is too large (%d vs %d max bytes)], $prop_key, $len, $prop_value->{maxSize} )
...
if (0) {
    my $response = HTTP::Tiny->new->get('https://github.com/bluesky-social/atproto/raw/main/lexicons/app/bsky/actor/defs.json');
    ...
    while ( my $next = $iter->() ) {
        At::Lexicon::Parser::lexicon( decode_json $next->slurp_utf8 ) if $next->is_file;
    }
}
use Data::Dump;
ddx At::Lexicon::app::bsky::actor::profileViewBasic->new( did => 'did:plc:z72i7hdynmk6r22z27h6tvur', handle => 'jay.bsky.social' );
exit;
ddx At::Lexicon::app::bsky::actor::adultContentPref->new( enabledd => 1 );
ddx At::Lexicon::app::bsky::actor::contentLabelPref->new( label    => 'todo', visibility => 'show' );
warn 'exit';
__END__
